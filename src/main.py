...
def get_start_date_from_user(gs_client: GSheetClient):
    ...
    last_date = (
        gs_client.sheet.cell(2, 1).value if gs_client.sheet.row_count > 1 else None
    )
    suggestion = f" [or press Enter for {last_date}]" if last_date else ""

    while True:
        date_str = console.input(
            f"[bold]Enter start date (YYYY-MM-DD){suggestion}: [/]"
        ).strip()
...
async def main():
    ...
    try:
        ...
        if "authorization key" in str(e) and "different IP addresses" in str(e):
            ...
            return  # Exit cleanly without trying to disconnect again
        else:
